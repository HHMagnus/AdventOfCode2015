...
fn main() {
	...
	let instructions = file.lines().map(|line| {
		if line.contains("hlf") {
			...
			Instruction::Jie(split.next().unwrap().chars().nth(4).unwrap(), split.next().unwrap().parse().unwrap())
		} else if line.contains("jio") {
			let mut split = line.split(", ");
			Instruction::Jio(split.next().unwrap().chars().nth(4).unwrap(), split.next().unwrap().parse().unwrap())
		} else {
